...
On a timeout, the order is usually still in progress, without any failing item: the report then also tells which order items are not done yet, and which state the service instance of each of them is hanging in (its lifecycle state, how many of its resources are deployed or still waiting, and the other services of the order it waits for).  That summary is part of the message of the `OrderStateTimeoutError` as well, and its `pending` attribute exposes it as objects.

If you handle the failures of the order yourself (e.g. by passing `bad_states=[]` and asserting on the order state in the test), you can trigger the same reporting on demand with `log_failures` and `log_pending`, or get each part of it as an object with `diagnose_failures`, `diagnose_non_compliance` and `diagnose_pending`.
...
```python
    ...
    # Log a summary of all the items of the order which are not done yet, and the state
    # the service instance of each of them is hanging in, or get them as objects
    order.log_pending()
    pending = order.diagnose_pending()
```
