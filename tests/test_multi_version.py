...
def test_basic_example_fork_compiler(testdir, module_venv_active):
    """The lsm_project fixture still works when the fork compile server is enabled."""

    utils.add_version_constraint_to_project(testdir.tmpdir)

    result = testdir.runpytest("tests/test_basics.py", "--lsm-fork-compiler")
    result.assert_outcomes(passed=1)
