...
- Add an `additional_services` argument to `LsmProject.post_partial_compile_validation`, to validate partial compiles which pull in services outside of the ownership tree of the service they are triggered for.  The resource sets of those services are then expected as well, and their resources are matched against the owned resource patterns.
