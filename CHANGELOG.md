...
- Report the diagnosis of every failing service instance when an order goes into a bad state or times out.  The new `RemoteOrder.log_failures` method and `remote_order.diagnose_failures` function allow to trigger the same reporting on demand, for orders whose failures are handled by the caller.
