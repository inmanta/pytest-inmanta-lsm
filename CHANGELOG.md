...
- Fix the report about the pending items of an order breaking on an orchestrator which reports the state of an order item as a plain string instead of an enum value (iso8).  A failure to build that report doesn't shadow the timeout it is about anymore.
- Honor a timeout which is shorter than the interval between two polls in `RemoteOrder.wait_for_state`: we used to sleep a full interval before looking at the timeout again, and to report an order which completed in the meantime as successful.
...
- Report the diagnosis of every failing service instance when an order goes into a bad state or times out.  The new `RemoteOrder.log_failures` and `RemoteOrder.diagnose_failures` methods allow to trigger the same reporting on demand, for orders whose failures are handled by the caller.
